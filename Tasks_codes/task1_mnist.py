...
# In[138]:
f=open("task1_catdog.txt","w")
f11=open("task_catdog_p.txt","w")
class NN:
    ...
    def load_data(self,path1,nameofset):    
        ...
        if nameofset=="Cat-Dog":
            ...
            print("jjjjj")								
            i=glob.glob("MNIST/0/*.jpg")
            #print(i)
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(0)
                #print(np.array(Y).shape)
            i=glob.glob("MNIST/1/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(1)
            i=glob.glob("MNIST/2/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(2)
            i=glob.glob("MNIST/3/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(3)
            i=glob.glob("MNIST/4/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(4)
            i=glob.glob("MNIST/5/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(5)
            i=glob.glob("MNIST/6/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(6)
            i=glob.glob("MNIST/7/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(7)
            i=glob.glob("MNIST/8/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(8)
            i=glob.glob("MNIST/9/*.jpg")
            for i1 in i:
                rdimg.append((cv2.imread(i1, cv2.IMREAD_GRAYSCALE)).ravel())
                Y.append(9)
...
#         self.rdimg = (rdimg1-mean)/std
        self.rdimg=rdimg
        mean,std   = np.array(self.rdimg).mean(axis=0),np.array(self.rdimg).std(axis=0)
        std = np.where(std==0,1,std)
        self.rdimg  = (self.rdimg-mean)/std
        return self.rdimg
    ...
    def swish_Activation_function(self,net_input,r=0):
        ...
        result1=result+ self.sigmoid_Activation_function(net_input,'false') * (1-result)
        if r==0:
            return result
        else:
            return result1
    
    def tanh(self,net_input,r=0):
        result=2*self.sigmoid(self,net_input,'false')-1
        result1=1-result**2
        if r==0:
            return result
        else:
...
for i in range(3,9):
    ...
    list1=[]
    k+=1
legends=['Total-Layer-count=3','Total-Layer-count=4','Total-Layer-count=5','Total-Layer-count=6','Layer-count=7','Total-Layer-count=8']
epochs=np.array(epochs).reshape(1,400)
...
plt.legend(legends)
plt.show()
plt.savefig(cwds+'/F1-MACRO_catdog.png')
plt.clf()
...
plt.legend(legends)
plt.show()
plt.savefig(cwds+'/F1-MICRO_catdog.png')
plt.clf()
...
plt.legend(legends)
plt.show()
plt.savefig(cwds+'/ACCURACY_catdog.png')
plt.clf()
...
plt.ylabel('F1-MACRO')
plt.legend("F1-macro")
plt.savefig(cwds+'/layer-vs-f1(macro).png')
plt.show()
plt.clf()

plt.plot(lcount,f2max)   
plt.xlabel('LAYER-COUNT')
plt.ylabel('F1-MICRO')
plt.legend("F1-micro")
plt.savefig(cwds+'/layer-vs-f1(micro).png')
plt.show()
plt.clf()

plt.plot(lcount,accmax)   
plt.xlabel('LAYER-COUNT')
plt.ylabel('ACCURACY')
plt.legend("ACCURACY")
plt.savefig(cwds+'/layer-vs-accuracy.png')
plt.show()
plt.clf()
